...
import { Download, FileText, RefreshCw, Package, FileSpreadsheet, AlertTriangle } from 'lucide-react';
import { billingService } from '../services/api';
import Table from '../components/Table';

const TopPartsReport = () => {
  ...
  const fetchTopParts = async () => {
    ...
    setError(null);
    try {
      const res = await billingService.getTopParts({ limite: 10 });
      ...
      const normalizedData = validData.map((item, index) => ({
        posicion: index + 1,
        nombre_repuesto: item.nombre_repuesto || item.nombre || 'Sin nombre',
      ...
      console.error('Error al cargar top repuestos:', error);
      setError('No se pudieron cargar los repuestos. Por favor, intenta nuevamente.');
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExportCSV = () => {
    ...
    try {
      const headers = ['Posición', 'Repuesto', 'Veces Usado', 'Total Ingresos (GTQ)'];
      ...
      link.setAttribute('download', `top_repuestos_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.text('Top 10 Repuestos Más Vendidos', pageWidth / 2, 28, { align: 'center' });
      ...
      const resumenY = 62;
      doc.text(`Total de repuestos analizados: ${data.length}`, 15, resumenY);
      ...
      doc.autoTable({
        startY: resumenY + 20,
        head: [['#', 'Repuesto', 'Veces Usado', 'Total Ingresos']],
        body: tableBody,
        theme: 'grid',
        headStyles: {
          fillColor: [37, 99, 235],
      ...
      doc.save(`top_repuestos_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (err) {
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      const wsData = [
        ['Posición', 'Repuesto', 'Veces Usado', 'Total Ingresos'],
        ...data.map(item => [
          item.posicion,
          item.nombre_repuesto,
          item.cantidad_vendida,
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Top Repuestos');
      const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
      const blob = new Blob([wbout], { type: 'application/octet-stream' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `top_repuestos_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-gray-600 mt-4 font-medium">Cargando top repuestos...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-blue-600 mb-1">Total de Repuestos</p>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2 text-sm text-gray-600">
          ...
          <span>Mostrando {data.length} repuestos más vendidos</span>
        </div>
